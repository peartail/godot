...
private:
	...
	real_t branch_junction_scale = 1.4;
	real_t branch_junction_length = 0.18;
	real_t leaf_spacing = 0.42;
	real_t leaf_size = 0.24;
	real_t leaf_width_scale = 0.62;
	real_t leaf_density = 1.0;
	int leaf_cluster_card_count = 3;
	real_t leaf_cluster_spread = 0.28;
	real_t leaf_size_variation = 0.16;
	real_t thorn_density = 0.0;
	real_t thorn_spacing = 0.55;
	real_t thorn_size = 0.09;
	...
	real_t bramble_radius = 1.4;
	real_t bramble_height = 1.0;
	int bramble_stem_count = 9;
	real_t bramble_tangle_strength = 0.7;
	real_t bramble_surface_bias = 0.55;
	real_t bramble_ground_anchor_ratio = 0.4;
...
public:
	...
	VINE_PROFILE_ACCESSOR(real_t, branch_junction_scale);
	VINE_PROFILE_ACCESSOR(real_t, branch_junction_length);
	...
	VINE_PROFILE_ACCESSOR(int, leaf_cluster_card_count);
	VINE_PROFILE_ACCESSOR(real_t, leaf_cluster_spread);
	VINE_PROFILE_ACCESSOR(real_t, leaf_size_variation);
	VINE_PROFILE_ACCESSOR(real_t, thorn_density);
	VINE_PROFILE_ACCESSOR(real_t, thorn_spacing);
	VINE_PROFILE_ACCESSOR(real_t, thorn_size);
	...
	VINE_PROFILE_ACCESSOR(real_t, bramble_radius);
	VINE_PROFILE_ACCESSOR(real_t, bramble_height);
	VINE_PROFILE_ACCESSOR(int, bramble_stem_count);
	VINE_PROFILE_ACCESSOR(real_t, bramble_tangle_strength);
	VINE_PROFILE_ACCESSOR(real_t, bramble_surface_bias);
	VINE_PROFILE_ACCESSOR(real_t, bramble_ground_anchor_ratio);
