...
static void _append_leaf(const Ref<SurfaceTool> &p_surface, const Vector3 &p_center, const Vector3 &p_tangent, const Vector3 &p_normal_hint, real_t p_size, real_t p_width_scale, real_t p_phase, real_t p_path_ratio, int p_side_sign) {
	...
	Vector3 outward = normal; Vector3 a = p_center - side * p_size * p_width_scale * 0.5; Vector3 b = p_center + side * p_size * p_width_scale * 0.5; Vector3 tip = p_center + p_tangent.normalized() * p_size + outward * p_size * 0.18;
	Color color(p_path_ratio, 0.8, 1.0, p_phase);
	_vine_vertex(p_surface, a, outward, Vector2(0, 0), color); _vine_vertex(p_surface, tip, outward, Vector2(0.5, 1), color); _vine_vertex(p_surface, b, outward, Vector2(1, 0), color);
	_vine_vertex(p_surface, b, -outward, Vector2(1, 0), color); _vine_vertex(p_surface, tip, -outward, Vector2(0.5, 1), color); _vine_vertex(p_surface, a, -outward, Vector2(0, 0), color);
}
...
Dictionary OpenWorldVineGenerator3D::validate_request() const {
	...
	if (generation_request.is_null()) add_error("REQUEST_MISSING", "assign generation_request.");
		...
		const bool needs_support = generation_request->get_explicit_anchors().is_empty() && generation_request->get_mode() != OpenWorldVineGenerationRequest::MODE_HANGING;
		...
		if (!generation_request->get_support_path().is_empty()) {
...
Ref<ArrayMesh> OpenWorldVineGenerator3D::_generate_lod_mesh(int p_lod) const {
	if (generated_path.is_null() || generation_request.is_null() || generation_request->get_profile().is_null()) return Ref<ArrayMesh>(); Ref<OpenWorldVineGenerationProfile> profile = generation_request->get_profile(); p_lod = CLAMP(p_lod, 0, 2); real_t quality = p_lod == 0 ? 1.0 : (p_lod == 1 ? profile->get_lod1_quality() : profile->get_lod2_quality()); int sides = p_lod == 2 ? 2 : MAX(3, (int)Math::round(profile->get_radial_sides() * Math::lerp((real_t)0.55, (real_t)1.0, quality))); real_t phase = Math::fposmod((real_t)(uint32_t)generation_request->get_seed() * (real_t)0.0000001192092896, (real_t)1.0);
	Ref<SurfaceTool> stem; stem.instantiate(); stem->begin(Mesh::PRIMITIVE_TRIANGLES); int path_stride = p_lod == 0 ? 1 : (p_lod == 1 ? 1 : 2);
	for (int path_index = 0; path_index < generated_path->get_path_count(); path_index++) { if (path_index > 0 && path_index % path_stride != 0) continue; _append_tube(stem, generated_path->get_path_points(path_index), profile->get_stem_radius() * (path_index == 0 ? 1.0 : 0.62), profile->get_tip_radius_scale(), sides, phase, path_index == 0 ? 0.3 : 1.0, generated_path->get_path_attached_flags(path_index), p_lod == 2); }
	stem->index(); stem->generate_tangents(); Ref<ArrayMesh> result_mesh = stem->commit();
	Ref<SurfaceTool> foliage; foliage.instantiate(); foliage->begin(Mesh::PRIMITIVE_TRIANGLES); int leaf_stride = p_lod == 0 ? 1 : (p_lod == 1 ? 2 : 4); RandomPCG random((uint64_t)(uint32_t)(generation_request->get_seed() ^ 0x5f3759df)); int leaf_count = 0;
	for (int path_index = 0; path_index < generated_path->get_path_count(); path_index++) { PackedVector3Array path = generated_path->get_path_points(path_index); PackedVector3Array path_normals = generated_path->get_path_normals(path_index); real_t accumulated = 0.0; real_t next_leaf = profile->get_leaf_spacing() * leaf_stride; for (int i = 1; i < path.size() - 1; i++) { accumulated += path[i - 1].distance_to(path[i]); if (accumulated < next_leaf) continue; next_leaf += profile->get_leaf_spacing() * leaf_stride; if (random.randf() > profile->get_leaf_density()) continue; Vector3 tangent = (path[i + 1] - path[i - 1]).normalized(); Vector3 normal = path_normals.size() == path.size() ? path_normals[i] : Vector3::UP; _append_leaf(foliage, path[i] + normal * profile->get_leaf_size() * 0.12, tangent, normal, profile->get_leaf_size() * (p_lod == 2 ? 1.35 : 1.0), profile->get_leaf_width_scale(), phase, (real_t)i / (path.size() - 1), leaf_count % 2 == 0 ? 1 : -1); leaf_count++; } }
	if (leaf_count > 0) { foliage->index(); foliage->generate_tangents(); foliage->commit(result_mesh); }
	result_mesh->set_name(vformat("StylizedVine_%d_%d_LOD%d", (int)generation_request->get_mode(), generation_request->get_seed(), p_lod)); return result_mesh;
}
