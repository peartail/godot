...
- 작성일: 2026-09-20. 상태: **구현 전 설계 초안**.
- 사용자가 요청한 산출물은 스펙 문서다. 이 문서 작성으로 구현·빌드·배포를 수행하지 않는다.
- 다음 세션은 이 문서를 기준으로 기술 검증부터 시작한다. 명령 이름과 내부 클래스명은 제안이다.
...
| 단계 | 범위 | 완료 조건 |
| --- | --- | --- |
| 0 | GDScript 동적 실행 기술 검증 | 컴파일 오류, return, await, 실행 오류와 객체 수명 확인 |
...
다음 항목은 구현 전 기술 검증으로 결정하며 지원된다고 가정하지 않는다.

1. C++ 작업 관리와 GDScript async 실행을 연결하는 최소 구현: GDScript 보조 실행기 또는 내부 API.
2. 실행 오류를 해당 job에 귀속하고 종료를 판정하는 방법. 전역 에디터 오류와 혼동하지 않을 것.
3. 최초 CLI 구현 언어와 배포 방식. 기존 Python/PowerShell 또는 엔진 실행 파일의 client mode 비교.
4. 세션 발견 파일의 Windows 사용자 접근 제어와 token 저장 방식.
5. 스크립트가 직접 에디터를 변경한 경우 generation·수정 상태 추적의 실제 한계.
6. 동시 GUI 편집 충돌 검출 범위와 대량 생성 시 프레임 응답성.

## 검증 및 다음 세션 인수인계

1. 저장소 AGENTS.md와 이 문서의 연결 문서를 읽고 기존 미커밋 변경을 보존한다.
2. 단계 0의 작은 실행 검증으로 async·오류 처리 가능성을 먼저 확인한다.
