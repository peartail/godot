...
- 단계 0 검증과 미결정 사항 정리를 마쳤다. 다음 세션은 단계 1을 시작한다. 명령 이름과 내부 클래스명은 제안이다.
...
아래 1·2·4·5·6은 [기술 검증](editor_automation_stage0_findings.md) 결과로, 3과 7은 결정으로 닫혔다.
**미결정 없음. 단계 1 착수 조건을 충족했다.**
...
### 4. 세션 발견 파일의 접근 제어와 token 저장 — 결정됨

**발견 파일 전체를 사용자 전용 경로에 둔다. 프로젝트 디렉터리는 쓰지 않는다.**

원안의 `.godot/agent/`는 안전하지 않다. 실측 결과 사용자의 실제 프로젝트 위치에서
`BUILTIN\Users`가 읽고 `Authenticated Users`가 수정할 수 있다. 상위 디렉터리에서 상속된 권한이며,
`.godot/`의 성질이 아니라 프로젝트를 어디 두었는가의 결과다. 위치를 통제할 수 없으므로 보장이 성립하지 않는다.

엔진은 이를 고치지 못한다. `FileAccessWindows::_set_unix_permissions()`는 `ERR_UNAVAILABLE`을 반환하며
Godot에 Windows ACL API가 없다. 보호는 경로 선택으로만 달성한다.

token만 옮기는 절충은 쓰지 않는다. 발견 파일을 고칠 수 있으면 `port`를 위조해 CLI를 가짜 서버로 보내고
token을 그대로 가져갈 수 있다. 연결 후 `session_id`·`project_path` 검증도 같은 파일에서 읽으므로 방어가 되지 않는다.

token 생성은 `Crypto.generate_random_bytes()`를 쓴다. mbedTLS PSA의 `psa_generate_random()`이다.

한계: 이 머신에서는 `user://`도 완전한 사용자 전용이 아니다(머신 정책 그룹에 읽기 권한이 있다).
더 강한 격리가 필요하면 네이티브 Win32 ACL 설정을 별도로 구현한다.
...
단계 0에서 새로 드러난 항목이다. **엔진을 수정하지 않는다. 문법 오류는 첫 하나만 보고한다.**

메시지 자체는 2번과 같은 경로로 얻는다. `reload()`는 코드만 반환하므로 `add_error_handler()`로 수집한다.

개수는 단계에 따라 다르다. 파싱 단계는 첫 오류 하나만 출력하고
(`gdscript.cpp:827`, `// TODO: Show all error messages.`), 분석 단계는 목록을 순회해 전부 출력한다
(`gdscript.cpp:848`). 잘리는 것은 문법 오류뿐이다.

문법 오류는 보통 첫 번째를 고치면 뒤따르는 오류가 연쇄로 사라지므로 하나만으로도 실용적이다.
반대로 서로 독립적이라 한꺼번에 보는 편이 유용한 분석 오류는 이미 전부 나온다.
엔진 수정은 upstream 머지 부담만 늘린다.
